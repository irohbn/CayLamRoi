...
async function addNewBook(req, rep) {
  ...
  //;
  await pump(req.body.image.toBuffer(), fs.createWriteStream(imgPath));
  const newBook = {
    image: imgPath,
...
async function updateBook(req, rep) {
  ...
  const { title, genre, description, price, author, page, code } = req.body;
  try {
    let updatedBook = await Book.findByIdAndUpdate(
      bookId,
      { title, genre, description, price, author, page, code },
      { new: true }
    );
